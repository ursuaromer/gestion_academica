...
matriculaRouter.post("/", async(req, res)=>{
    const {id, firstName, lastName1, lastName2, nacimiento, dni, direccion, telefono, email, programaEstudio} = req.body
    try {
        const newMatricula = await createMatriculaController({id, firstName, lastName1, lastName2, nacimiento, dni, direccion, telefono, email, programaEstudio})
