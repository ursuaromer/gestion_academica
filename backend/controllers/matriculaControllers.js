...
const createMatriculaController =  async ({id, firstName, lastName1, lastName2, nacimiento, dni, direccion, telefono, email, programaEstudio, ciclo}) => {
    try {
        const newMatricula = await Matricula.create({id, firstName, lastName1, lastName2, nacimiento, dni, direccion, telefono, email, programaEstudio, ciclo})
        return newMatricula
    } catch (error) {
